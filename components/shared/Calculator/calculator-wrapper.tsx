...
import { ButtonLocation, History, DIGITS, OPERATORS, SPECIAL } from "../../../constants/calculator";
...
export const CalculatorWrapper = () => {
  ...
  const [focusedLocation, setFocusedLocation] = useState<ButtonLocation | null>(null);
  const [layoutRefs, setLayoutRefs] = useState<RefObject<HTMLButtonElement>[][]>(() => {
    const layoutRefs = new Array(layout.length);
    layout.forEach(
      (element, idx) => (layoutRefs[idx] = new Array(element.length).fill(createRef()))
    );
    return layoutRefs;
  });
  ...
  useEventListener("keydown", (e) => {
    ...
    if (!(e instanceof KeyboardEvent) || isHistoryOpened || e.ctrlKey || e.altKey) return;
  ...
  const initButton = (rowIdx: number, columnIdx: number, ref: RefObject<HTMLButtonElement>) => {
  ...
  return (
    <div
      ...
      className="relative flex min-h-[707px] w-[400px] flex-col overflow-hidden rounded-2xl bg-[#F1F2F3] shadow-2xl"
    >
      {isHistoryOpened && (
        <FocusTrap
          focusTrapOptions={{
        ...
        >
          <div className="absolute top-0 z-10 flex h-full w-full flex-col gap-4 bg-slate-100 p-5 text-slate-900">
            <div className="flex items-start justify-between">
              ...
              <button onClick={closeHistory} className="focus-visible:outline-[#000]">
                <svg
            ...
            {history.length > 0 ? (
                  ...
                  {history.map(({ firstNumber, secondNumber, operator, answer }, index) => {
                    ...
                    return (
                      <button
                        key={key}
                        onClick={(e) => {
                          applyState(firstNumber, secondNumber, operator, answer);
                          closeHistory(e);
                        }}
                        className="rounded-md border-2 border-white bg-white px-4 py-2 text-right shadow focus-visible:outline-[#000]"
                      >
              ...
              <div className="flex flex-1 flex-col items-center justify-center gap-2 bg-[]">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                ...
                <h2 className="mb-2 text-xl">Ничего не найдено</h2>
                <button
                  onClick={closeHistory}
                  className="mb-8 rounded-full bg-violet-200 px-6 py-2 text-sm text-[#F1F2F3] duration-75 hover:bg-violet-400 hover:text-white"
                >
                  Начать расчет
                </button>
              </div>
      ...
      <button
        onClick={openHistory}
        className="mt-2 flex items-center justify-center gap-1 self-center rounded-full bg-slate-100 px-4 py-1 text-xs text-slate-900 focus-visible:outline-[#F1F2F3]"
      >
        История
        <svg
          xmlns="http://www.w3.org/2000/svg"
